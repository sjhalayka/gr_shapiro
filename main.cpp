...
void idle_func(void)
{
	...
	{
		const long double delta_shapiro_time = -2 * grav_constant * sun_mass / pow(speed_of_light, 3.0);
		long double delta_shapiro_time_simple = delta_shapiro_time;
		long double delta_shapiro_time_complicated = delta_shapiro_time;
		...
		delta_shapiro_time_simple *= log(1.0 - R.dot(X));


		const long double R_sun = (custom_math::vector_3(-span * speed_of_light, sun_radius, 0) - custom_math::vector_3(0, 0, 0)).length();
		const long double R_half_span = (custom_math::vector_3(-span * speed_of_light, sun_radius, 0) - custom_math::vector_3(0, sun_radius, 0)).length();

		// https://www.qeios.com/read/IVCVBM
		delta_shapiro_time_complicated *= log((R_sun + R_half_span)/(R_sun - R_half_span));

		total_time *= 2.0;
		long double straight_time = 2.0 * (custom_math::vector_3(-span * speed_of_light, sun_radius, 0) - custom_math::vector_3(span * speed_of_light, sun_radius, 0)).length() / speed_of_light;

		long double delta_shapiro_time2 = (total_time - straight_time);

		cout << delta_shapiro_time_simple << " " << delta_shapiro_time_complicated << " " << delta_shapiro_time2 << endl;
		cout << delta_shapiro_time_complicated / delta_shapiro_time2 << endl;
